...
# Acorn / LiteFury underside (RHS Research "contents" photo): fan end left, M.2 edge right, connectors on the bottom edge.
...
end.rotate(-90, expand=True).save(OUT / "acorn-cw.jpg", quality=90)   # connectors on the LEFT edge, P2 on top, pin 1 at the bottom
end.rotate(90, expand=True).save(OUT / "acorn-ccw.jpg", quality=90)   # connectors on the RIGHT edge, P1 on top, pin 1 at the top
...
hat.rotate(90, expand=True).save(OUT / "hat-ccw.jpg", quality=92)   # header on the RIGHT edge, pin 1 on top (pinout-chart order)
...
W, H = full.size
full.crop((int(W * 0.59) + 80, int(H * 0.55) + 10, int(W * 0.59) + 1720, int(H * 0.55) + 520)).save(OUT / "blade-port.jpg", quality=90)
