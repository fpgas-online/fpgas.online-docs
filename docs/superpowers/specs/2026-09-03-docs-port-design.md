...
| `acorn-pinmap.md` says the 2026-08-31 P2 survey used "the fixed pin-ID bitstream" with the release asset's filename; `acorn-pcie-programming.md` says that release asset predates PR #10 and never toggles a pin. One is wrong, or the survey used a locally rebuilt file. | `boards/acorn/pcie-programming.md`, `boards/acorn/wiring.md` |
| `acorn-pcie-programming.md`'s SRAM bootstrap and initial-setup procedures omit the PCIe rescan after the JTAG load and the Pi 5 `gpiochip15` symlink; both added on the docs page during Task 11 with a note. Fix upstream. | `boards/acorn/pcie-programming.md` |
...
| Fomu EVT: `fomu-evt.md` says DPRAM is 15 × 8 Kbit blocks, `fomu-pin-mapping.md` says 30 EBR blocks (the iCE40UP5K datasheet figure is 30 × 4 Kbit); the fleet programs with `openFPGALoader -b fomu` while the spec teaches `dfu-util`. Recorded in Task 16. | `boards/fomu-evt.md` |
...
| `verify_hardware.py` (origin/main, read 2026-09-04): `poe_reset()` matches `pi(\d+)$` against host keys that are all `welland-piNN`, `ps1-piNN` or `rpi[35]-netv2`, so it always returns False and the Fomu DFU-timeout recovery is unreachable; `GATEWAYS` still reaches Welland as `pi@tweed.welland.mithis.com`, an account gone with the 2026-08-30 reinstall. Code bugs, fix upstream in test-designs. Found in Task 26. | `setup/verification.md` |
| `verify-hardware.md` drifts from the code: `ssh_type` is `gateway` with a `GATEWAYS` table, not `tweed`; `EXTRA_UPLOADS` has two entries, not three; no Acorn entry although `PROGRAM_CMD["acorn"]` exists; OpenOCD config is `~/netv2/alphamax-rpi.cfg`; the Welland NeTV2 pool runs OpenOCD without `sudo` while `rpi3-netv2` uses it; `--gpio-release` for the PMOD design is undocumented; `variant` is not NeTV2-only (`welland-pi2` acorn has `cle-215+`); its JTAG pin-order gloss `TCK:TDO:TDI:TMS` is wrong (openFPGALoader takes TDI:TDO:TCK:TMS, as `netv2.md` and `acorn-pinmap.md` say). Found in Task 26. | `setup/verification.md` |
| `deployment-checklist.md` says `web.yml --tags ttsite` renders `tt-boards.yaml` for the gateway and the Pi NFS root; the Pi copy is baked in by `onpi/tasks/tt.yml`. Its Welland port ranges (TT 1-10, Acorns 29/43-48) do not match the Welland tables (TT on 3-8, no Acorn on p45). Found in Task 26. | `setup/verification.md` |
...
| `fpgas.online-site`'s README lists `pibdemos` as a live app, but it is not in `INSTALLED_APPS`, not routed by either `urls.py`, and infra renders no nginx include for it; the demo buttons work by typing into the WebSSH iframe (`demos.js`). The README also names only `TTSITE_COMMANDER_VERSION`; code and role carry a second `TTSITE_COMMANDER_LEGACY_VERSION` bundle. Also: the classic upload form is broken (`pibup/forms.py` has `run` commented out, `views.py` still reads it, so every POST raises KeyError); `/pistat/` views are csrf_exempt and proxied unconditionally, so anyone can post a status line or trigger a ping; `pibfpgas.views.one` looks up boards by port alone; `fpga.html` hard-codes the legacy `vlc .../live/pi<N>.m3u8` URL. Noted in Task 24. | `setup/webapp.md` |
...
| `onpi/tasks/apt.yml` on main installs Debian's openfpgaloader 0.10.0; the `openfpgaloader-rp1pio` swap is only on infra PR #48. `core_freq=500` is not set anywhere (TECHDEBT records the PoE-vs-camera tradeoff undecided). `fpgas-pistat-cam.service` binds to `cam.service`, which no longer exists (`fpgas-cam.service`). `pistat-scripts/*.py` in the setup-pi deb are server-side (Django channels, `/srv/www/pib/venv` shebang). The `fpgas-arty-*` units are also unrunnable as shipped: their `ExecStart` names `/usr/local/bin/arty_here.sh` while nfpm installs `fpgas-arty-here.sh` (same for the `.exp` and for wire/blink), and they order after non-existent targets. | `setup/pi.md` |
...
| The VLAN-per-port design spec numbers the S3300 as switch 1 and the GSM7252PS as switch 2; the deployed host_vars and the Welland page have them the other way (GSM7252PS = sw1 head). The spec also says trunk 49/`access_ports: 48` on both; LLDP shows GSM 1/0/47 and 1/0/50, 40 access ports on sw1. The prototype runbook follows the spec. | `setup/network.md` |
| Security hygiene, not a docs item: `ansible/inventory/host_vars/fpgas.online.yml` records both Welland switch SNMP community strings in a plaintext comment next to the vaulted variables, in a public repository. The docs do not repeat them. Rotate and remove upstream. | (infra repo) |
| `poe.sh` is unconverged at Welland: `site/tasks/snmp.yml` is guarded by `switch.mpi_port is defined`, a field the per-port scheme removed, and `poe.sh` sources `/etc/environment.export` (singular) while the role writes `/etc/environment.exports`; PS1 works by history. | `setup/network.md` |
| `group_vars/all/srv.yml` pins `dist: bookworm` for every host and nothing in the infra repo builds a trixie root, yet `sites/ps1.md` documents the Compute Blades booting `/srv/nfs/rpi/trixie/` arm64. How that root is built and maintained is unrecorded. Noted in Task 19. | `setup/netboot.md`, `sites/ps1.md` |
...
| NeTV2: `netv2-pin-mapping.md` says rpi5-netv2 programs with openFPGALoader rp1pio today, `netv2.md` calls RP1 PIO JTAG future, and the 2026-03-09 Welland survey found only OpenOCD installed; the UART test args say `/dev/ttyAMA0` while the same page puts rpi3 on `/dev/ttyS0`; the NeTV2 schematic PDF has no online URL; for the production Pi 3B+ hosts `site-welland.md` says OpenOCD while `netv2.md` says openFPGALoader libgpiod; test-designs also carries `alphamax-rpi5-sysfsgpio.cfg`, an OpenOCD Pi 5 path. Recorded as todos in Task 15. | `boards/netv2.md` |
| The hardware README's deployment counts disagree with the site pages: Welland pending TT02/TT03/TT09 vs the Welland page naming only reserved ports for tt09/tt10; NeTV2 (RPi5 PCIe) `(+×4)` and ButterStick/ULX3S `(+×4)` at Welland with no trace on the Welland page; PS1 TT ASIC eight pending vs seven. Noted on `boards/index.md`. | `boards/index.md`, `sites/welland.md` |
